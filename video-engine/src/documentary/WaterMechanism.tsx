...
export const Pump:React.FC<{t:number;wire?:boolean;stopWindow:{start:number;end:number}}>=({t,wire=false,stopWindow})=>{
 const {start,end}=stopWindow, duration=end-start;
 const stop=ramp(t,start,end);
 const elapsed=clamp((t-start)/duration);
 const angle=t<start?t*230:start*230+115*duration*(1-Math.pow(1-elapsed,2));
